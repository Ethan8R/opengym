...
// `features.restTimerPush === false` means the host physically can't do this — a serverless
// deployment has no process left alive when the rest is over — so don't call at all rather than
// fire a request that is bound to fail. Absent config (older server, or offline) means yes.
const restPushOK = () => {
  const { user, config } = useStore.getState()
  return !!user && config?.features?.restTimerPush !== false
}
const pushRestTimer = sec => { if (restPushOK()) api('/api/push/rest-timer', { method: 'POST', body: JSON.stringify({ seconds: sec }) }).catch(() => {}) }
const cancelPushRestTimer = () => { if (restPushOK()) api('/api/push/rest-timer/cancel', { method: 'POST', body: '{}' }).catch(() => {}) }
